...
public class LibraryCardAppletFixed extends Applet {
     ...
     */
    protected LibraryCardAppletFixed() {
    ...
    public static void install(byte[] bArray, short bOffset, byte bLength) {
        LibraryCardAppletFixed applet = new LibraryCardAppletFixed();
        
        // Khởi tạo các biến
        applet.pin = new OwnerPIN(PIN_TRY_LIMIT, PIN_MAX_SIZE);
        applet.cardId = new byte[CARD_ID_LENGTH];
        applet.holderName = new byte[NAME_MAX_LENGTH];
        applet.expiryDate = new byte[8];
        applet.borrowedBooks = new byte[MAX_BORROWED_BOOKS * BOOK_ID_LENGTH];
        applet.numBorrowedBooks = 0;
        applet.cardInitialized = false;
        applet.holderNameLength = 0;
